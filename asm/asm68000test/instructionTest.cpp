...
namespace instructionTest
{
	...
	BOOST_AUTO_TEST_CASE(and_long_number)
	{
		asmparser parser;
		auto opcode = parser.parseText("  and.l   #$5FFFFF,d1\n");
		validate_noErrors(parser);
		const auto& code = validate_codeSize(parser, 3);
		BOOST_CHECK_EQUAL(0xc2bc, code[0]);
		BOOST_CHECK_EQUAL(0x005f, code[1]);
	...
	BOOST_AUTO_TEST_CASE(bcc_all)
	{
		asmparser parser;
		auto opcode = parser.parseText("  nop\n  bcc $20\n  bcs $20\n  beq $20\n  bne $20\n  bge $20\n  bgt $20\n  bhi $20\n  ble $0\n  bls $0\n  blt $0\n  bmi $0\n  bpl $0\n  bvc $0\n  bvs $0\n  bra $0\n");
		const auto& code = validate_codeSize(parser, 16);
	...
	BOOST_AUTO_TEST_CASE(bit_immediate)
	{
		...
		const auto& code = validate_codeSize(parser, 10);
	...
	BOOST_AUTO_TEST_CASE(dbcc_all)
	{
		...
		const auto& code = validate_codeSize(parser, 2*17);
	...
	// MOVE USP
	// ====================================================================================================
	BOOST_AUTO_TEST_CASE(move_fromUsp)
	{
		asmparser parser;
		auto opcode = parser.parseText("  move USP,a3\n");
		validate_hasValue<uint16_t>(0b0100'1110'0110'1'011, opcode);
	}
	BOOST_AUTO_TEST_CASE(move_toUsp)
	{
		asmparser parser;
	...
	// MOVE SR
	// ====================================================================================================
	BOOST_AUTO_TEST_CASE(move_fromSr)
	{
		asmparser parser;
		auto opcode = parser.parseText("  move SR,(a3)\n");
		validate_hasValue<uint16_t>(0b0100'0000'11'010'011, opcode);
	}
	BOOST_AUTO_TEST_CASE(move_fromSrError)
	{
		asmparser parser;
		auto opcode = parser.parseText("  move SR,a0\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	BOOST_AUTO_TEST_CASE(move_toSr)
	{
		asmparser parser;
		auto opcode = parser.parseText("  move -(a2),SR\n");
		validate_hasValue<uint16_t>(0b0100'0110'11'100'010, opcode);
	}
	BOOST_AUTO_TEST_CASE(move_toSrError)
	{
		asmparser parser;
		auto opcode = parser.parseText("  move a6,SR\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	// ====================================================================================================
	// MOVEM
	...
	// MOVEP
	// ====================================================================================================
	BOOST_AUTO_TEST_CASE(movep_toMemory)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep.l d3, 8(a1)\n");
		validate_hasValue<uint16_t>(0b0000'011'111'001'001, opcode);

		const auto& code = validate_codeSize(parser, 2);
		BOOST_CHECK_EQUAL(8, code[1]);
	}
	BOOST_AUTO_TEST_CASE(movep_toMemoryDefault)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep d5, 8(a3)\n");
		validate_hasValue<uint16_t>(0b0000'101'110'001'011, opcode);

		const auto& code = validate_codeSize(parser, 2);
		BOOST_CHECK_EQUAL(8, code[1]);
	}
	BOOST_AUTO_TEST_CASE(movep_toMemoryFailed)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep.b d3, 8(a1)\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	BOOST_AUTO_TEST_CASE(movep_fromMemory)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep.l 42(a3), d6\n");
		validate_hasValue<uint16_t>(0b0000'110'101'001'011, opcode);

		const auto& code = validate_codeSize(parser, 2);
		BOOST_CHECK_EQUAL(42, code[1]);
	}
	BOOST_AUTO_TEST_CASE(movep_fromMemoryDefault)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep 24(a7), d0\n");
		validate_hasValue<uint16_t>(0b0000'000'100'001'111, opcode);

		const auto& code = validate_codeSize(parser, 2);
		BOOST_CHECK_EQUAL(24, code[1]);
	}
	BOOST_AUTO_TEST_CASE(movep_fromMemoryFailed)
	{
		asmparser parser;
		auto opcode = parser.parseText("  movep.b 8(a1), d5\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	// ====================================================================================================
	// MOVEQ
	...
	{
		asmparser parser;
		auto opcode = parser.parseText("  trap #20\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	...
	{
		asmparser parser;
		auto opcode = parser.parseText("  trapv\n");
		validate_hasValue<uint16_t>(0b0100'1110'0111'0110, opcode);
	}

	// ====================================================================================================
	// TST
	// ====================================================================================================
	BOOST_AUTO_TEST_CASE(tst_byte)
	{
		asmparser parser;
		auto opcode = parser.parseText("  tst.b d2\n");
		validate_hasValue<uint16_t>(0b0100'1010'00'000'010, opcode);
	}
	BOOST_AUTO_TEST_CASE(tst_word)
	{
		asmparser parser;
		auto opcode = parser.parseText("  tst.w (a3)\n");
		validate_hasValue<uint16_t>(0b0100'1010'01'010'011, opcode);
	}
	BOOST_AUTO_TEST_CASE(tst_default)
	{
		asmparser parser;
		auto opcode = parser.parseText("  tst 3(a3)\n");
		validate_hasValue<uint16_t>(0b0100'1010'01'101'011, opcode);
	}
	BOOST_AUTO_TEST_CASE(tst_long)
	{
		asmparser parser;
		auto opcode = parser.parseText("  tst.l -(a1)\n");
		validate_hasValue<uint16_t>(0b0100'1010'10'100'001, opcode);
	}
	BOOST_AUTO_TEST_CASE(tst_failed)
	{
		asmparser parser;
		auto opcode = parser.parseText("  tst #1234\n");
		BOOST_CHECK_EQUAL(1, parser.getErrors().get().size());
	}
	// ====================================================================================================
	// UNLK
